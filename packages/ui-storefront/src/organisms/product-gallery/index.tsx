...
function ProductGalleryMain({ className, showControls = true, ...props }: ProductGalleryMainProps) {
  ...
  return (
    <div
      className={cn(
        'relative aspect-square md:aspect-[4/5] flex-1 bg-muted rounded-[var(--radius-lg)] overflow-hidden',
        className,
      )}
      {...props}
    >
      <img
        ...
        src={activeImage.src}
        alt={activeImage.alt}
        className="w-full h-full object-cover animate-in fade-in duration-[var(--motion-normal)] ease-[var(--motion-ease-out)]"
      />
        ...
        <>
          <div className="absolute inset-y-0 left-4 flex items-center">
            <IconButton
              ...
              onClick={prevImage}
              className="bg-background/80 backdrop-blur shadow-sm hover:bg-background transition-all"
            />
          </div>
    ...
    >
      {images.map((image, idx) => (
        <button
          key={image.id}
          className={cn(
            'relative snap-start shrink-0 aspect-square rounded-[var(--radius-md)] overflow-hidden border-2 transition-all duration-[var(--motion-fast)]',
