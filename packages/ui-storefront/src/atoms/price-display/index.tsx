...
function PriceDisplay({ 
  ...
  return (
    <div className={cn("flex items-center flex-wrap gap-2", className)} {...props}>
      <span className={cn("font-bold tracking-tight text-foreground", hasSale && 'text-brand', textSizes.price)}>
        {formatter.format(price)}
      </span>
      {hasSale && (
        <>
          <span className={cn("text-muted-foreground line-through decoration-muted-foreground/50 font-medium translate-y-[1px]", textSizes.original)}>
