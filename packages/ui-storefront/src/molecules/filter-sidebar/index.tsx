          ...
          <div className="space-y-3">
            {group.options?.map((opt) => (
              <label key={opt.value} className="flex items-center gap-3 cursor-pointer group/label">
                <Checkbox 
                  onCheckedChange={(c) => onFilterChange?.(group.id, { value: opt.value, checked: c })}
                />
                <span className="text-sm font-medium text-muted-foreground group-hover/label:text-foreground transition-colors flex-1">{opt.label}</span>
                {opt.count !== undefined && <span className="text-xs text-muted-foreground/70">{opt.count}</span>}
              </label>
            ))}
          </div>
        )}
          ...
          <div className="flex flex-wrap gap-2">
            {group.options?.map((opt) => (
              <label 
                ...
                title={opt.label}
              >
                <input type="checkbox" className="peer sr-only" onChange={(e) => onFilterChange?.(group.id, { value: opt.value, checked: e.target.checked })} />
                <div 
                  className={cn(
...
function FilterSidebar({ groups, onFilterChange, onClearAll, className, ...props }: FilterSidebarProps) {
  return (
