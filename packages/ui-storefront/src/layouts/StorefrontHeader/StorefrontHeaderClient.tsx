...
import { Button, cn, Dropdown, DropdownTrigger, DropdownContent, DropdownItem } from '@ecom/ui'
  ...
  return (
    <header
      className={cn(
        ...
        isScrolled
          ? 'bg-background/85 backdrop-blur-xl shadow-[0_1px_0_0_var(--border-subtle),var(--elevation-floating)] py-2'
          : 'bg-background/95 py-4 border-b border-border/40',
        className,
      )}
      {...restProps}
    >
        ...
        <div className="flex items-center justify-between gap-4 md:gap-8">
          ...
          <div className="flex items-center gap-1 sm:gap-3 shrink-0">
            ...
            >
              ...
              {wishlistCount > 0 && (
                <span className="absolute top-1 right-1 w-4 h-4 bg-foreground text-background text-[var(--space-3)] font-bold rounded-full flex items-center justify-center pointer-events-none group-hover:bg-brand transition-colors">
            ...
            >
              ...
              {cartCount > 0 && (
                <span className="absolute top-1 right-1 w-4 h-4 bg-brand text-brand-foreground text-[var(--space-3)] font-bold rounded-full flex items-center justify-center pointer-events-none ring-2 ring-background">
          ...
          >
            {categories.map((cat, i) => (
              <a
                key={i}
                href={cat.href}
                className="px-4 py-1.5 rounded-full text-xs font-semibold text-muted-foreground hover:text-foreground hover:bg-muted transition-all duration-[var(--motion-fast)] uppercase tracking-wider"
              >
                {cat.label}
              </a>
            ))}
