import { Button, Typography } from '@ecom/core-ui'
import { Plus, Ticket } from 'lucide-react'
import type { CampaignsProps } from '../..'
import { CAMPAIGNS_EMPTY_MESSAGE } from '../Campaigns.constants'

interface EmptyProps {
  newCampaignLabel: string
  onNewCampaign?: CampaignsProps['onNewCampaign'] | undefined
}

export function Empty({ newCampaignLabel, onNewCampaign }: EmptyProps) {
  return (
    <div className="border-border bg-muted/20 flex min-h-72 flex-col items-center justify-center rounded-2xl border border-dashed p-8 text-center">
      <div className="bg-primary-soft text-primary mb-4 flex size-12 items-center justify-center rounded-2xl">
        <Ticket className="size-6" />
      </div>

      <Typography variant="body-sm" className="font-semibold">
        {CAMPAIGNS_EMPTY_MESSAGE}
      </Typography>

      <Typography variant="caption" className="text-muted-foreground mt-2 max-w-sm">
        Create a campaign to start increasing customer engagement and redemptions.
      </Typography>

      <Button type="button" className="mt-5" onClick={() => void onNewCampaign?.()}>
        <Plus className="size-4" />
        {newCampaignLabel}
      </Button>
    </div>
  )
}
