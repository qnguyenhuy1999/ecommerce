...
export async function createNotification(data: Record<string, unknown>) {
  return api<{ success: boolean; data: NotificationListItem }>('/admin/notifications', {
    method: 'POST',
...
export async function sendNotification(id: string) {
  return api<{ success: boolean }>(`/admin/notifications/${id}/send`, { method: 'POST' })
}

export async function getTemplates() {
  return api<{ success: boolean; data: NotificationTemplate[] }>('/admin/notifications/templates')
}
